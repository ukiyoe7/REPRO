...
CARTOES_ALELO_0422 <- get(load("C:\\Users\\Repro\\Documents\\R\\ADM\\CAMPANHAS\\2022\\ABR\\CARTOES_ALELO_0422.R"))
...
View(CP_G139_0422)
CP_G139_0422 %>%  summarize(v=sum(VRVENDA)) # total
CP_G139_0422 %>%  summarize(v=sum(VRVENDA)*0.07) # vendedoras 
CP_G139_0422 %>%  summarize(v=sum(VRVENDA)*0.03) # montador
sum(CP_G139_0422 %>%  summarize(v=sum(VRVENDA)*0.07),CP_G139_0422 %>%  summarize(v=sum(VRVENDA)*0.03)) #total

### Cria Planilha identificação CPFs

CP_G139_0422_IPEDIDOS <- CP_G139_0422 %>% group_by(ID_PEDIDO,PEDDTBAIXA) %>% summarize(VRVENDA=sum(VRVENDA)) %>% 
  as.data.frame() %>% `colnames<-`(c("ID_PEDIDO","DATA","VALOR.VENDA")) %>% 
  mutate(DATA=format(DATA,"%d/%m/%y")) 

range_write("184OAqDxeie-fRlZgCOCMRUD7vJIGWJM8Gc9HSKhDWt8",data=CP_G139_0422_IPEDIDOS,sheet = "ABR22",
            range = "A1") 

## obtem dados planilha identificação CPFs

CPF_G139_CPF_0422 <- read_sheet("184OAqDxeie-fRlZgCOCMRUD7vJIGWJM8Gc9HSKhDWt8",sheet = "ABR22") %>% 
  as.data.frame() %>% 
  mutate(CPF=sub("\\D+", '',CPF)) %>% 
  mutate(CPF=sub("\\.", '',CPF)) %>% 
  mutate(CPF=sub("\\-", '',CPF))

View(CPF_G139_CPF_0422)


#lista pedidos

LIST_G139_0422 <- inner_join(CP_G139_0422,CPF_G139_CPF_0422,by="ID_PEDIDO") %>% 
  mutate(OBS="SCHROEDER G139 04/22") %>% .[,c(-12,-13,-14)]
...
# Calculo vendedoras
PAG_G139_0422_VENDEDORAS <- LIST_G139_0422 %>% group_by(CPF) %>% summarize(BONUS=sum(BONUS)) 

#calculo com montador

PAG_G139_0422_MONTADOR <- data.frame(CPF=c("88717020930"),BONUS=(CP_G139_0422 %>% summarize(BONUS=sum(VRVENDA)*0.03))) 


PAG_G139_0422 <- left_join(rbind(PAG_G139_0422_VENDEDORAS,PAG_G139_0422_MONTADOR) ,CARTOES_ALELO_0422 %>% 
                             select(NSERIE,CPF),by="CPF") %>%
  .[,c(3,1,2)] %>% mutate(OBS="SCHROEDER G139 04/22") %>%  mutate(CLIENTE='') %>% 
  mutate(GRUPO='139') 

View(PAG_G139_0422)

PAG_G139_0422 %>% summarize(BONUS=sum(BONUS)) 


### ===========================================================================================
## SQL 849 VITAL

CP_849_0422 <- dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
PD AS (SELECT ID_PEDIDO,PEDDTBAIXA,PEDID.CLICODIGO,GCLCODIGO,CLINOMEFANT,SETOR,PEDAUTORIZOU 
       ...
       INNER JOIN FIS ON PEDID.FISCODIGO1=FIS.FISCODIGO
       INNER JOIN CLI ON PEDID.CLICODIGO=CLI.CLICODIGO
       WHERE PEDDTBAIXA BETWEEN '01.04.2022' AND '30.04.2022'
      AND PEDSITPED<>'C' AND PEDLCFINANC IN ('S', 'L','N'))
...
CP_849_0422 %>% summarize(v=sum(VRVENDA))

CP_849_0422 %>% summarize(v=sum(VRVENDA)*0.07)
...
LIST_849_0422 %>% summarize(v=sum(BONUS))
...
PAG_849_0422 <- left_join(data.frame(CPF=c("04455447911","06532582913"),
                                     BONUS=(CP_849_0422 %>%
...
CP_157_0422 %>% summarize(v=sum(VRVENDA))

CP_157_0422 %>% summarize(v=sum(VRVENDA)*0.1)


## lista pedidos 
LIST_157_0422 <- CP_157_0422 %>% 
  mutate(CPF=rep(c("03188282940"), length.out=nrow(CP_157_0422))) %>% mutate(OBS="DOM BOSCO 157 04/22")


View(LIST_157_0422)

LIST_157_0422 %>% summarize(v=sum(BONUS))


LIST_157_0422 %>% group_by(CLICODIGO,CPF) %>% summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(DATA=floor_date(Sys.Date(),"month") %m-% months(1)) 


## Pagamento

PAG_157_0422 <- left_join(LIST_157_0422 %>% group_by(CPF) %>% summarize(BONUS=round(sum(BONUS),2)),
                          CARTOES_ALELO_0422 %>% 
                            select(NSERIE,CPF),by="CPF") %>% 
  .[,c(3,1,2)] %>%  mutate(OBS="DOM BOSCO 157 04/22")  %>%  
...
CP_BC_G37_0422 <-dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
PD AS (SELECT ID_PEDIDO,PEDDTBAIXA,PEDID.CLICODIGO,GCLCODIGO,SETOR,CLINOMEFANT,PEDAUTORIZOU 
       ...
       INNER JOIN CLI ON PEDID.CLICODIGO=CLI.CLICODIGO
       WHERE 
       PEDDTBAIXA BETWEEN  '01.04.2022' AND '30.04.2022'
       AND PEDSITPED <>'C'),
...
CP_BC_G37_0422 %>% summarize(BONUS=sum(BONUS)) 

### LISTA PEDIDOS

LIST_BC_G37_0422 <- CP_BC_G37_0422 %>% filter(!nchar(CPF)<11)  %>% mutate(OBS="AUGUSTO G37 04/22")

View(LIST_BC_G37_0422)

LIST_BC_G37_0422_2 <- CP_BC_G37_0422 %>% mutate(OBS="AUGUSTO G37 04/22")

View(LIST_BC_G37_0422_2)


CP_BC_G37_0422  %>% group_by(CPF) %>% summarize(BONUS=sum(BONUS)) 

### Pagamentos vendedoras

PAG_BC_G37_0422 <- left_join(CP_BC_G37_0422 %>%
                               filter(!nchar(CPF)<11) %>% 
                               group_by(CPF) %>% 
                               summarize(BONUS=round(sum(BONUS),2)),
                             CARTOES_ALELO_0422 %>% 
                               select(NSERIE,CPF),by="CPF") %>%.[,c(3,1,2)] %>% 
  mutate(OBS="AUGUSTO G37 04/22") %>% 
  mutate(CLIENTE='') %>%  mutate(GRUPO='37')

View(PAG_BC_G37_0422)

## Calculo bonificacao gerente

PAG_BC_G37_0422_GERENTE <- left_join(
  data.frame(CPF='06833523932',BONUS=CP_BC_G37_0422 %>% summarize(BONUS=sum(QTD)/2*5)),
  CARTOES_ALELO_0422 %>% 
    select(NSERIE,CPF),by="CPF") %>%.[,c(3,1,2)] %>% 
  mutate(OBS="AUGUSTO G37 04/22") %>% 
  mutate(CLIENTE='') %>%  mutate(GRUPO='37')

View(PAG_BC_G37_0422_GERENTE)

## Calculo coordenadora loja 


PAG_BC_G37_0422_COORD <- left_join(
...
View(PAG_BC_G37_0422_COORD )


PAG_BC_G37_0422_4 <- union(PAG_BC_G37_0422,PAG_BC_G37_0422_GERENTE) %>% 
  union(.,PAG_BC_G37_0422_COORD)
View(PAG_BC_G37_0422_4)
...
PD AS (SELECT ID_PEDIDO,PEDDTBAIXA,PEDID.CLICODIGO,GCLCODIGO,CLINOMEFANT,SETOR,PEDAUTORIZOU 
       ...
       INNER JOIN CLI ON PEDID.CLICODIGO=CLI.CLICODIGO
       WHERE 
       PEDDTBAIXA BETWEEN '01.04.2022' AND '30.04.2022' AND PEDSITPED <>'C'),
...
CP_G45_0422 %>% summarize(v=sum(BONUS))


## LISTA PEDIDOS
LIST_G45_0422 <- CP_G45_0422 %>% 
  mutate(CPF=rep(c("82323097920"), length.out=nrow(CP_G45_0422))) %>% 
  mutate(OBS="G45 OTICA DINIZ 04/22")

View(LIST_G45_0422)

CP_G45_0422 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(DATA=floor_date(Sys.Date(),"month") %m-% months(1)) 
...
PAG_G45_0422 <- left_join(LIST_G45_0422 %>% group_by(CPF) %>% 
                            summarize(BONUS=round(sum(BONUS),2)),
                          CARTOES_ALELO_0422 %>% 
                            select(NSERIE,CPF),by="CPF") %>% 
  .[,c(3,1,2)] %>% 
...
CP_360_0422 %>% summarize(v=sum(BONUS))

#LISTA PEDIDOS
LIST_360_0422 <- CP_360_0422  %>% mutate(OBS="360 OTICA UNIVERSAL 04/22")

View(LIST_360_0422)


CP_360_0422 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(DATA=floor_date(Sys.Date(),"month") %m-% months(1)) 

## pagamentos

PAG_360_0422 <- left_join(CP_360_0422 %>% group_by(CPF) %>% 
                            summarize(BONUS=round(sum(BONUS),2)),
                          CARTOES_ALELO_0422 %>% 
                            select(NSERIE,CPF),by="CPF") %>% .[,c(3,1,2)] %>% 
  mutate(OBS="360 OTICA UNIVERSAL 04/22") %>%  mutate(CLIENTE='360') %>%  mutate(GRUPO='') 

View(PAG_360_0422)

PAG_360_0422 %>% summarize(v=sum(BONUS))

## ====================================================================================

### SQL 213 OTICA LINO sem resultado

CP_213_0422 <- dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
CLI  AS(SELECT c.CLICODIGO,GCLCODIGO,CLINOMEFANT,SETOR FROM CLIEN C
       LEFT JOIN (SELECT CLICODIGO,D.ZOCODIGO,ZODESCRICAO SETOR FROM ENDCLI D
       INNER JOIN ZONA Z ON D.ZOCODIGO=Z.ZOCODIGO WHERE ENDFAT='S')A ON C.CLICODIGO=A.CLICODIGO
...
CP_213_0422 %>% summarize(v=sum(BONUS))

## distribui os CPFs dos participantes nos pedidos
CP_213_0422_2 <- CP_213_0422 %>% 
  mutate(CPF=rep(c("91212766920","06528427984","02235457088","03336880904"), 
                 length.out=nrow(CP_213_0422)))

View(CP_213_0422_2)


LIST_213_0422 <- CP_213_0422_2  %>% mutate(OBS="213 OTICA LINO 04/22") 
View(LIST_213_0422)


CP_213_0422_2 %>% group_by(CLICODIGO,CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(DATA=floor_date(Sys.Date(),"month") %m-% months(1)) %>% 
  mutate(BONUS=round(sum(BONUS),2)/4)


PAG_213_0422<- left_join(CP_213_0422_2 %>% group_by(CLICODIGO,CPF) %>% 
                           summarize(BONUS=round(sum(BONUS),2)) %>% 
                           mutate(BONUS=round(sum(BONUS),2)/4), CARTOES_ALELO_0422 %>% 
                           select(NSERIE,CPF),by="CPF") %>% .[,c(4,2,3)] %>%
  mutate(OBS="OTICA LINO 213 04/22") %>%
...
View(CP_1419_0422)
CP_1419_0422 %>% summarize(v=sum(BONUS))

### lista pedidos  


LIST_1419_0422<- CP_1419_0422 %>% mutate(CPF=rep(c("47751118091"), length.out=nrow(CP_1419_0422))) %>% 
...
PAG_1419_0422 <- left_join(LIST_1419_0422
...
INNER JOIN FIS ON PEDID.FISCODIGO1=FIS.FISCODIGO
INNER JOIN CLI ON PEDID.CLICODIGO=CLI.CLICODIGO
WHERE PEDDTBAIXA BETWEEN '01.04.2022' AND '30.04.2022' AND PEDSITPED <>'C'),
...
CP_305_0422 %>% summarize(v=sum(BONUS))


### lista pedidos

LIST_305_0422 <- CP_305_0422 %>% 
  mutate(CPF=rep(c("66655072972"), length.out=nrow(CP_305_0422))) %>% 
  mutate(OBS="305 OTICA GARUVA 04/22")
...
PAG_305_0422 <- left_join(LIST_305_0422 %>% group_by(CPF) %>% 
                            summarize(BONUS=round(sum(BONUS),2)),
                          CARTOES_ALELO_0422 %>% 
                            select(NSERIE,CPF),by="CPF") %>% .[,c(3,1,2)] %>% 
  mutate(OBS="305 OTICA GARUVA 04/22")  %>% 
...
View(PAG_305_0422)
PAG_305_0422 %>% summarize(v=sum(BONUS)) 

## =============================================================================================================         

# SQL G121 RINALDI

CP_G121_0422 <- dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
CP_G121_0422 %>% summarize(v=sum(VRVENDA))

CP_G121_0422 %>% summarize(v=sum(BONUS))

### lista pedidos  

LIST_G121_0422 <- CP_G121_0422 %>%  mutate(OBS="RINALDI G121 04/22")


### pagamentos

PAG_G121_0422 <- left_join(CP_G121_0422 %>% group_by(CPF) %>% summarize(BONUS=round(sum(BONUS),2)),
                           CARTOES_ALELO_0422 %>% 
                             select(NSERIE,CPF),by="CPF") %>% 
  .[,c(3,1,2)] %>% 
  mutate(OBS="RINALDI G121 04/22") %>% 
  mutate(CLIENTE='') %>%  mutate(GRUPO='121') 

View(PAG_G121_0422)

PAG_G121_0422 %>% summarize(v=sum(BONUS))



##  SQL 2183 OTICA MADRID --- outros produtos sem resulrtado

CP_2183_0422_2 <- dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
View(CP_2183_0422_2)
CP_2183_0422_2 %>% summarize(v=sum(BONUS))
CP_2183_0422_2 %>% filter(nchar(CPF)==11) %>% summarize(v=sum(BONUS))

### lista pedidos  


LIST_2183_0422_2<- CP_2183_0422_2 %>% filter(nchar(CPF)==11) %>%  mutate(OBS="2183 OTICA MADRID OUTROS 04/22")
View(LIST_2183_0422_2)


## agrupa bonificação CPf

CP_2183_0422_2 %>% group_by(CLICODIGO,CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(DATA=floor_date(Sys.Date(),"month") %m-% months(1)) 

### pagamentos  

PAG_2183_0422_2 <- left_join(CP_2183_0422_2 %>% 
                               filter(nchar(CPF)==11) %>%  group_by(CLICODIGO,CPF) %>% 
                               summarize(BONUS=round(sum(BONUS),2)) , CARTOES_ALELO_0422 %>% 
                               select(NSERIE,CPF),by="CPF") %>% .[,c(4,2,3)] %>%
  mutate(OBS="2183 OTICA MADRID OUTROS 04/22") %>% 
  mutate(CLIENTE='2183') %>%  mutate(GRUPO='') 


View(PAG_2183_0422_2)
...
CP_206_0422 %>% summarize(v=sum(VRVENDA))

CP_206_0422 %>% summarize(v=sum(BONUS))

### lista pedidos  

LIST_206_0422 <- CP_206_0422 %>% 
  mutate(CPF=rep(c("06850418912"), length.out=nrow(CP_206_0422))) %>% 
  mutate(OBS="206 OTICA RH 04/22")


### pagamentos

PAG_206_0422 <- left_join(LIST_206_0422 %>% group_by(CPF) %>% summarize(BONUS=round(sum(BONUS),2)),
                          CARTOES_ALELO_0422 %>% 
                            select(NSERIE,CPF),by="CPF") %>% .[,c(3,1,2)] %>% 
  mutate(OBS="206 OTICA RH 04/22") %>% 
...
CP_2183_0422_1 <- dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
View(CP_2183_0422_1)
CP_2183_0422_1 %>% summarize(v=sum(BONUS))
CP_2183_0422_1 %>% filter(nchar(CPF)==11) %>% summarize(v=sum(BONUS))

### lista pedidos  

LIST_2183_0422_1 <- CP_2183_0422_1 %>% filter(nchar(CPF)==11) %>%  
  mutate(OBS="2183 OTICA MADRID INSIGNE 04/22")
View(LIST_2183_0422_1)


## agrupa bonificação CPf

CP_2183_0422_1 %>% group_by(CLICODIGO,CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(DATA=floor_date(Sys.Date(),"month") %m-% months(1)) 

### pagamentos  

PAG_2183_0422_1 <- left_join(CP_2183_0422_1 %>% 
                               filter(nchar(CPF)==11) %>%  group_by(CLICODIGO,CPF) %>% 
                               summarize(BONUS=round(sum(BONUS),2)) , CARTOES_ALELO_0422 %>% 
                               select(NSERIE,CPF),by="CPF") %>% .[,c(4,2,3)] %>%
  mutate(OBS="2183 OTICA MADRID INSIGNE 04/22") %>% 
...
CP_INSIGNE_0422 <-dbGetQuery(con2,"
...
View(PAG_INSIGNE_0422)

PAG_INSIGNE_0422 %>% as.data.frame() %>%  summarize(b=sum(BONUS))

PAG_INSIGNE_0422 %>% .[duplicated(.$CPF),] %>% View()
...
PAG_ALL_0422_2 <- PAG_ALL_0422 %>% group_by(CPF,NSERIE) %>% 
  summarize(BONUS=sum(BONUS))

View(PAG_ALL_0422_2)

PAG_ALL_0422_2 %>% summarize(v=sum(BONUS))

sheet_write(PAG_ALL_0422_2,ss="1mpH2ottgRXHrTpCM0LCj9Dq-uuC4aMEsuRE5fuK7te4",sheet="ALELO")  


PAG_ALL_0422_2 %>% .[duplicated(.$CPF),]
...
LIST_ALL_0422 <-  rbind( 
  LIST_G139_0422,   ## schroeder ok
  LIST_849_0422,    ## Vital ok
  LIST_157_0422,    ## Dom bosco OK
  LIST_360_0422,    ## Universal ok
  LIST_1419_0422,   ##look ok
  LIST_305_0422,    ## Garuva  OK
  LIST_G121_0422,   ## rinaldi OK
  LIST_206_0422 ## RH 
) 
...
write.csv2(LIST_ALL_0422,file = "C:/Users/Repro/Documents/R/2022/CAMPANHAS/FEV/LIST_ALL_0422.CSV")

LIST_ALL_0422 %>% summarize(v=sum(BONUS))


sheet_write(LIST_ALL_0422,ss="1MXoghPoQ8MXe9e8W-XADgKJDf_44k-L3IA6lTPZCKSw",sheet="PEDIDOS")       
...
CP_TOTAL_0422 <- PAG_ALL_0422 %>% summarize(TOTAL=sum(BONUS))    

range_write("1MXoghPoQ8MXe9e8W-XADgKJDf_44k-L3IA6lTPZCKSw",data=CP_TOTAL_0422,sheet = "RESUMO",
            range = "A1")


CP_TOTAL_0422_CLIENTE <- PAG_ALL_0422 %>% filter(is.na(GRUPO)) %>% 
  group_by(CLIENTE) %>% rename("COD"="CLIENTE") %>% summarize(BONUS=sum(BONUS))


CP_TOTAL_0422_CLIENTE %>%  mutate(TIPO='LOJA')  %>% View()

range_write("hhjfP6tEgQDV9UUl8F4-KPMexYU67gdYwj_-A7vMl4",data=CP_TOTAL_0422_CLIENTE,sheet = "RESUMO",
            range = "A5")

CP_TOTAL_0422_GRUPO <- PAG_ALL_0422 %>% filter(!is.na(GRUPO)) %>% 
  group_by(GRUPO) %>%  rename("COD"="GRUPO") %>%  summarize(BONUS=sum(BONUS))


CP_TOTAL_0422_GRUPO %>% filter(str_detect(COD,"\\d+")) %>% mutate(TIPO='GRUPO')  %>% View()


range_write("hhjfP6tEgQDV9UUl8F4-KPMexYU67gdYwj_-A7vMl4",data=CP_TOTAL_0422_GRUPO,sheet = "RESUMO",
            range = "D5")
...
LIST_ALL_SETORES_0422 <-  rbind( 
  LIST_G139_0422,     ## schroeder ok
  LIST_849_0422,      ## Vital ok
  LIST_157_0422,      ## Dom bosco OK
  LIST_BC_G37_0422_2, ## Barbara K OK
  LIST_4295_0422,     ## Lumita OK
  LIST_G45_0422,      ## Diniz OK 
  LIST_360_0422,      ## Universal ok
  LIST_213_0422,      ## Lino OK
  LIST_1419_0422,     ## lOOK OK  
  LIST_305_0422,      ## Garuva  OK
  LIST_G121_0422,     ## rinaldi OK
  LIST_2183_0422_2,   ## Madrid insigne ok
  LIST_206_0921_0422, ## RH 
  LIST_G195_0821_0122_2,## Luna ok
  LIST_1390_0122,     ##testoni
  LIST_G146_1221_0122,## Marcio Pinheiro ok
  LIST_INSIGNE_0422_SETORES,## Insigne
  LIST_RETRO_0122_2,
...
View(LIST_ALL_SETORES_0422)


sheet_write(LIST_ALL_SETORES_0422,ss="1dirGCDZQmlNTnZfC_gQhiXKE9DAoRNc-2HNWkSfego4",sheet="DADOS")       
